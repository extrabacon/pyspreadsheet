...
/**
* Set row properties (height, format, options)
* @param {Number} row Row index
* @param {Object} settings settings->height, settings->format, settings->options
* @returns {SpreadsheetWriter} The same writer instance for chaining calls
*
* more usage info at http://xlsxwriter.readthedocs.org/worksheet.html#set_row
*/

SpreadsheetWriter.prototype.set_row = function (row, settings) {
    this.state.currentSheetIndex == -1 && this.addSheet();
    this.pyshell.send(['set_row', row, settings]);
    return this;
};
...
* @param {Object} settings settings->width, settings->format, settings->options, settings->last_col
* @returns {SpreadsheetWriter} The same writer instance for chaining calls
*
* more usage info at http://xlsxwriter.readthedocs.org/worksheet.html#set_column
*/

SpreadsheetWriter.prototype.set_column = function (col_index, settings) {
    this.state.currentSheetIndex == -1 && this.addSheet();
    this.pyshell.send(['set_column', col_index, settings]);
    return this;
};
...
SpreadsheetWriter.prototype.write_rich_string = function (row, col, parts) {
