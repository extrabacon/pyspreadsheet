 ...
 * @param {String} [path] The path where to save the resulting file - ignore to receive the stream instead
 * @param {Function} [callback] The callback function to invoke with the resulting stream - ignore to write and save
 * data manually with the returned SpreadsheetWriter instance
 * @returns {SpreadsheetWriter} The SpreadsheetWriter instance to use to write the file
 */
exports.write = function (workbook, path, callback) {

	if (_.isFunction(path)) {
		callback = path;
		path = null;
	} else if (_.isString(path) && !callback) {
		callback = function () {};
	}
	...
	if (path && callback) {
		...
		writer.save(function (err, stream) {
			if (err) {
				...
				stream.pipe(fs.createWriteStream(path), callback);
			}
		});
	} else if (callback) {
